...
## 2026-09-05 — Grund 1/3/4 и SFI A/B: 5 параллельных агентов
...
`schema.ts`, эталонный урок `grund-2/transport` как образец, конкретные темы, правило
«не уверен в форме слова — не пиши её»), без доступа к истории этой сессии.

- **Grund 1** — Тело (kroppen), Дом (hemma, комнаты и мебель), Животные (djur).
- **Grund 3** — Деньги и цены (числа 20–100 — раньше их не было нигде, только 0–20),
  Здоровье/визит к врачу, Профессии.
- **Grund 4** — Жильё и аренда, Госучреждения и бланки (personnummer, Skatteverket,
  Försäkringskassan), Эмоции.
- **SFI A** — Алфавит (с явным акцентом на å/ä/ö как отдельные буквы в конце алфавита,
...
- **SFI B** — Семья и отношения (глубже a1/family: статус в браке, родственники со
...
- Финальная сквозная проверка после всех агентов: typecheck, 39 тестов, валидация
  ...
  (урок → квиз, без ошибок консоли), обзор `/tracks` — счётчики уроков сходятся с учётом
  того, что часть уроков A1 из первой партии помечена сразу и в grund-1/sfi-a.
  Закоммичено 5 коммитами (по уровню).

---

## 2026-09-05 — Перепроверка программ SFI и SVA, грамматика по уровням

Пересверили названия и структуру уровней с тем, как это устроено у Skolverket, и
обнаружили две настоящие ошибки в `CURRICULUM.md`, а не только вопрос написания.

- **Названия.** «SFI Курс A» → **SFI kurs A** (официальное написание — `kurs` строчными).
  «Grund 1–4» → **SVA grund delkurs 1–4**; полное название трека —
  `svenska som andraspråk, grundläggande nivå`.
- **Ошибка 1: SVA grund — это не параллель SFI, а следующая ступень.** Файл утверждал,
  что «Grund 1 идёт параллельно с SFI A/B». На деле это курс komvux на 700 poäng
  (delkurs 1/2/3/4 = 100/200/200/200 p), вход в delkurs 1 — законченный SFI kurs D,
  а delkurs 4 даёт behörighet к гимназическому `SVA 1`.
- **Ошибка 2: не было studievägar.** Без них непонятно, почему kurs A нужен не всем:
  studieväg 1 — A→B→C→D, studieväg 2 — B→C→D, studieväg 3 — C→D.
- Пересчитано соответствие CEFR: SFI kurs D — это A2 (а не «почти B1»), SVA grund
  delkurs 4 — B1+ (а не «мостик к SFI C/D»). Добавлена сквозная таблица соответствий.
- Темы SVA delkurs 1–4 подняты до реального уровня: вместо повторного «цвета/животные» —
  рассказ о прошлом, аргументативный текст, referat и källkritik, трудовое право,
  формальные письма. Все уже написанные уроки в списках сохранены, ничего не осиротело.
- **К каждому из 11 уровней добавлено по 20 пунктов грамматики** — 220 позиций,
  выстроенных сквозной прогрессией: то, что на SFI даётся образцами, на SVA delkurs 1
  систематизируется с терминами, а к delkurs 4 доходит до компрессии придаточных,
  хеджирования и языковой правки.

Технически:

- `content/tracks.json` — обновлены `title` уровней; **ID (`sfi-a`, `grund-1`, …) не
  тронуты**, чтобы не ломать пути `content/lessons/<level>/` и поле `levels` уроков.
  Трек SVA получил свой `note`.
- `TracksPage` больше не хардкодит примечание под треком SFI: рендерит `track.note` из
  JSON для любого трека. Неиспользуемый ключ `tracks.sfi.note` убран из обеих локалей.
- `SPEC.md`/`SPEC_ru.md`/`README.md` приведены к новым названиям; в SPEC теперь три
  таксономии, а не две.
- В `TODO.md` записано **известное расхождение**: семь уроков `a1/*` кросс-помечены как
  `grund-1`, а собственные уроки `grund-1`/`grund-2` по уровню ближе к SFI kurs A/B.
  Контент намеренно не тронут — нужно выбрать между (a) снять кросс-тег `grund-1` и
  (b) перенести уроки в `sfi-a`/`sfi-b` и писать delkurs 1 заново под A2.
- Проверка: typecheck, 39 тестов, валидация контента (26 уроков) — всё зелёное.
