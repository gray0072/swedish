A question changes shape once it's embedded inside another sentence — the
question word stays, but the word order switches from question order to
subordinate-clause order.

## Direct versus indirect

```example
Var bor du? — Where do you live? (direct: verb right after var)
Jag undrar var du bor. — I wonder where you live. (indirect: subject before
the verb)
```

In the direct question, the finite verb comes immediately after the
question word. Once the question is embedded — after **undra**, **fråga**,
**veta**, and similar verbs — the subject moves in front of the verb, just
like any subordinate clause.

```example
Vad heter hon? → Vet du vad hon heter? — What's her name? → Do you know
what her name is?
Vad var klockan? → Hon frågade vad klockan var. — What time was it? → She
asked what time it was.
```

## No question word: use om

A yes/no question with no question word is embedded with the conjunction
**om** ("whether/if").

```example
Kommer han? → Jag vet inte om han kommer. — Is he coming? → I don't know if
he's coming.
```

## When the question word is the subject: add som

If the question word would itself be the subject of the direct question
(*Vem ringde?*), the embedded clause needs **som** right after it, or the
sentence would be missing a subject entirely.

```example
Vem ringde? → Ingen vet vem som ringde. — Who called? → No one knows who
called.
Vad hände? → Polisen undersöker vad som hände. — What happened? → The
police are investigating what happened.
```

## In short

Embedding a question keeps its question word (or adds **om**), but always
switches to subordinate word order — subject before verb — and inserts
**som** whenever the question word stands in for the subject.
